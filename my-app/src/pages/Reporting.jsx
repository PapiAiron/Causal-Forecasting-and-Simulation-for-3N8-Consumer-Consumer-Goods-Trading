            ...
            {reportTypes.map((type) => {
              ...
              return (
                ...
                >
                  <Card
                    className={`p-6 ${
                      selectedPeriod === type.id ? "ring-2" : ""
                    }`}
                    style={
                      selectedPeriod === type.id
                        ? { borderColor: type.color, borderWidth: "2px" }
                        : {}
                    }
                  >
                    ...
                    <tbody>
                      {fullReports.monthly.map((month, idx) => {
                        ...
                        const growth = prevMonth
                          ? ((month.total_sales - prevMonth.total_sales) /
                              prevMonth.total_sales) *
                            100
                          : 0;
                        return (
                          <tr
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                {fullReports.yearly.map((year, idx) => {
                  ...
                  const growth = prevYear
                    ? ((year.total_sales - prevYear.total_sales) /
                        prevYear.total_sales) *
                      100
                    : 0;
                  return (
                    <div
                      key={idx}
                      className="p-6 rounded-xl bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 border-2 border-amber-200 dark:border-amber-800"
                    >
                      ...
                      <div className="text-3xl font-bold text-amber-900 dark:text-amber-300 mt-2">
                        {year.total_sales.toLocaleString()}
                      </div>
                      {prevYear && (
                        <div
                        ...
                        >
                          {growth > 0 ? "+" : ""}
                          {growth.toFixed(1)}% vs previous year
                        </div>
                      )}
                    </div>
                  );
            ...
            <Card className="p-6 mb-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      ...
                      <div className="text-sm text-yellow-700 dark:text-yellow-400 mt-1">
                        {categoryAnalysis.top_sku.brand} •{" "}
                        {categoryAnalysis.top_sku.bottle_size}
                      </div>
                    </div>
                    <div>
                      <div className="text-sm font-medium text-yellow-800 dark:text-yellow-400">
                      ...
                      <div className="text-xs text-yellow-700 dark:text-yellow-400 mt-1">
                        {categoryAnalysis.top_sku.cases?.toLocaleString() || 0}{" "}
                        cases
                      </div>
                    </div>
                    <div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
                ...
                <div>
                  ...
                  {categoryAnalysis?.brands &&
                  categoryAnalysis.brands.length > 0 ? (
                    <>
                      <ResponsiveContainer width="100%" height={300}>
                        <PieChart>
                          <Pie
                            ...
                            outerRadius={100}
                            label={(entry) => {
                              const total = categoryAnalysis.brands.reduce(
                                (sum, b) => sum + b.sales,
                                0
                              );
                              const pct = ((entry.sales / total) * 100).toFixed(
                                1
                              );
                ...
                <div>
                  <h4 className="text-md font-semibold text-gray-800 dark:text-gray-200 mb-4">
                    Sales by Bottle Size
                  </h4>

                  {categoryAnalysis?.bottle_sizes &&
                  categoryAnalysis.bottle_sizes.length > 0 ? (
                    <>
                      <ResponsiveContainer width="100%" height={300}>
                        <BarChart
                          data={categoryAnalysis.bottle_sizes}
                          layout="vertical"
                        >
                          <CartesianGrid
                            strokeDasharray="3 3"
                      ...
                      <div className="mt-6 space-y-3">
                        {categoryAnalysis.bottle_sizes.map((size, idx) => {
                          const total = categoryAnalysis.bottle_sizes.reduce(
                            (sum, s) => sum + s.sales,
                            0
                          );
                          const percentage = (
                  ...
                  <div className="mb-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                      {categoryAnalysis.categories.map((cat, idx) => {
                        const total = categoryAnalysis.categories.reduce(
                          (sum, c) => sum + c.sales,
                          0
                        );
                        const percentage = ((cat.sales / total) * 100).toFixed(
                          1
                        );
                        return (
                          <div
            ...
            <Card className="p-12">
              <div className="text-center text-gray-500 dark:text-gray-400">
                ...
                <p className="text-sm mb-4">
                  Upload your sales data in Causal Analysis to generate
